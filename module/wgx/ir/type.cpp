...
TypeTable::LayoutInfo TypeTable::GetLayoutInfo(TypeId id,
  ...
  switch (type->kind) {
    ...
    case TypeKind::kVector: {
      ...
      // both std140 and std430 has the same layout for array vector:
      // vec2 = 8 bytes, align 8; vec3 = 12 bytes, align 16; vec4 = 16 bytes,
      // align 16. Arrays/matrices round vec3 stride up to 16 separately, but a
      // following scalar member may reuse the last component slot in the same
      // 16-byte register.
      if (num_components == 2) {
