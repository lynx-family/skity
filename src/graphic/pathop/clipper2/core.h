...
constexpr int64_t MAX_COORD = INT64_MAX >> 2;
constexpr int64_t MIN_COORD = -MAX_COORD;
constexpr int64_t INVALID = INT64_MAX;
...
template <typename T>
struct is_round_invocable<T,
                          std::void_t<decltype(std::round(std::declval<T>()))>>
    : std::true_type {};
...
struct Point {
  ...
  inline void Init(const T2 x_ = 0, const T2 y_ = 0) {
    if constexpr (std::is_integral_v<T> && is_round_invocable<T2>::value &&
...
inline Rect<T1> ScaleRect(const Rect<T2>& rect, double scale) {
  ...
  if constexpr (std::is_integral_v<T1> && is_round_invocable<T2>::value &&
                !std::is_integral_v<T2>) {
...
template <typename T1, typename T2>
inline Paths<T1> ScalePaths(const Paths<T2>& paths, double scale_x,
                            double scale_y, int& error_code) {
  Paths<T1> result;
...
inline Paths<T1> TransformPaths(const Paths<T2>& paths) {
  Paths<T1> result;
  std::transform(
      paths.cbegin(), paths.cend(), std::back_inserter(result),
...
template <typename T>
inline bool NearEqual(const Point<T>& p1, const Point<T>& p2,
                      double max_dist_sqrd) {
  return Sqr(p1.x - p2.x) + Sqr(p1.y - p2.y) < max_dist_sqrd;
}

template <typename T>
inline Path<T> StripNearEqual(const Path<T>& path, double max_dist_sqrd,
                              bool is_closed_path) {
...
template <typename T>
inline Paths<T> StripNearEqual(const Paths<T>& paths, double max_dist_sqrd,
                               bool is_closed_path) {
  Paths<T> result;
  result.reserve(paths.size());
  for (typename Paths<T>::const_iterator paths_citer = paths.cbegin();
    ...
    result.emplace_back(
        std::move(StripNearEqual(*paths_citer, max_dist_sqrd, is_closed_path)));
  }
  return result;
}
...
inline void CheckPrecisionRange(int& precision, int& error_code) {
  ...
  precision =
      precision > 0 ? CLIPPER2_MAX_DEC_PRECISION : -CLIPPER2_MAX_DEC_PRECISION;
}
...
template <typename T>
inline int CrossProductSign(const Point<T>& pt1, const Point<T>& pt2,
                            const Point<T>& pt3) {
...
template <typename T>
inline bool IsCollinear(const Point<T>& pt1, const Point<T>& sharedPt,
                        const Point<T>& pt2)  // #777
{
...
template <typename T>
inline double CrossProduct(const Point<T>& pt1, const Point<T>& pt2,
                           const Point<T>& pt3) {
  return (
      static_cast<double>(pt2.x - pt1.x) * static_cast<double>(pt3.y - pt2.y) -
...
template <typename T>
inline double DotProduct(const Point<T>& pt1, const Point<T>& pt2,
                         const Point<T>& pt3) {
  return (
      static_cast<double>(pt2.x - pt1.x) * static_cast<double>(pt3.x - pt2.x) +
...
inline bool GetSegmentIntersectPt(const Point<T>& ln1a, const Point<T>& ln1b,
                                  const Point<T>& ln2a, const Point<T>& ln2b,
                                  Point<T>& ip) {
  ...
  if constexpr (std::is_integral_v<T>) {
    int64_t originx =
        (CC_MIN(bb0maxx, bb1maxx) + CC_MAX(bb0minx, bb1minx)) >> 1;
    int64_t originy =
        (CC_MIN(bb0maxy, bb1maxy) + CC_MAX(bb0miny, bb1miny)) >> 1;
    ...
    double originx =
        (CC_MIN(bb0maxx, bb1maxx) + CC_MAX(bb0minx, bb1minx)) / 2.0;
    double originy =
        (CC_MIN(bb0maxy, bb1maxy) + CC_MAX(bb0miny, bb1miny)) / 2.0;
    double ln0c = (ln1dy * static_cast<double>(ln1a.x - originx)) +
                  (ln1dx * static_cast<double>(ln1a.y - originy));
    double ln1c = (ln2dy * static_cast<double>(ln2a.x - originx)) +
                  (ln2dx * static_cast<double>(ln2a.y - originy));
...
inline bool SegmentsIntersect(const Point64& seg1a, const Point64& seg1b,
  ...
  if (inclusive) {
    ...
    return (GetSign(CrossProduct(seg1a, seg2a, seg2b)) *
                GetSign(CrossProduct(seg1b, seg2a, seg2b)) <
            0) &&
           (GetSign(CrossProduct(seg2a, seg1a, seg1b)) *
                GetSign(CrossProduct(seg2b, seg1a, seg1b)) <
            0);
  }
}
