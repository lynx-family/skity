...
// Every closed path (ie polygon) is made up of a series of vertices forming
// edge 'bounds' that alternate between ascending bounds (containing edges going
// up relative to the Y-axis) and descending bounds. 'Local Minima' refers to
...
inline int64_t TopX(const Active& ae, const int64_t currentY) {
  if ((currentY == ae.top.y) || (ae.top.x == ae.bot.x))
    ...
    return ae.bot.x +
           static_cast<int64_t>(nearbyint(ae.dx * (currentY - ae.bot.y)));
  // nb: std::nearbyint (or std::round) substantially *improves* performance
  // here as it greatly improves the likelihood of edge adjacency in
...
inline double AreaTriangle(const Point64& pt1, const Point64& pt2,
                           const Point64& pt3) {
  return (
      static_cast<double>(pt3.y + pt1.y) * static_cast<double>(pt3.x - pt1.x) +
...
void AddLocMin(LocalMinimaList& list, Vertex& vert, PathType polytype,
               bool is_open) {
...
void AddPaths_(const Paths64& paths, PathType polytype, bool is_open,
  ...
  const auto total_vertex_count = std::accumulate(
      paths.begin(), paths.end(), size_t(0),
...
void ReuseableDataContainer64::AddLocMin(Vertex& vert, PathType polytype,
                                         bool is_open) {
  // make sure the vertex is added only once ...
  if ((VertexFlags::LocalMin & vert.flags) != VertexFlags::Empty) return;

  vert.flags = (vert.flags | VertexFlags::LocalMin);
  minima_list_.emplace_back(
      std::make_unique<LocalMinima>(&vert, polytype, is_open));
}

void ReuseableDataContainer64::AddPaths(const Paths64& paths, PathType polytype,
                                        bool is_open) {
...
void ClipperBase::Reset() {
  if (!minima_list_sorted_) {
    std::stable_sort(minima_list_.begin(), minima_list_.end(),
                     LocMinSorter());  // #594
...
void ClipperBase::AddPaths(const Paths64& paths, PathType polytype,
                           bool is_open) {
...
void ClipperBase::AddReuseableData(
  ...
  for (i = reuseable_data.minima_list_.cbegin();
       i != reuseable_data.minima_list_.cend(); ++i) {
    minima_list_.emplace_back(std::make_unique<LocalMinima>(
...
void ClipperBase::SetWindCountForClosedPathEdge(Active& e) {
  ...
  // find the nearest closed path edge of the same PolyType in AEL (heading
  // left)
...
bool IsValidAelOrder(const Active& resident, const Active& newcomer) {
  ...
  if (!IsMaxima(resident) && (resident.top.y > newcomer.top.y)) {
    return (CrossProductSign(newcomer.bot, resident.top,
                             NextVertex(resident)->pt) <= 0);
  } else if (!IsMaxima(newcomer) && (newcomer.top.y > resident.top.y)) {
    return (CrossProductSign(newcomer.bot, newcomer.top,
  ...
  else if (IsCollinear(PrevPrevVertex(resident)->pt, resident.bot,
                       resident.top))
    return true;
  else
    // compare turning direction of the alternate bound
    return (CrossProductSign(PrevPrevVertex(resident)->pt, newcomer.bot,
...
void ClipperBase::InsertLocalMinimaIntoAEL(int64_t bot_y) {
  ...
  while (PopLocalMinima(bot_y, local_minima)) {
    if ((local_minima->vertex->flags & VertexFlags::OpenStart) !=
        VertexFlags::Empty) {
      left_bound = nullptr;
    } else {
    ...
    if ((local_minima->vertex->flags & VertexFlags::OpenEnd) !=
        VertexFlags::Empty) {
      right_bound = nullptr;
    } else {
    ...
    if (right_bound) {
      ...
      while (right_bound->next_in_ael &&
        ...
        IntersectEdges(*right_bound, *right_bound->next_in_ael,
                       right_bound->bot);
...
OutPt* ClipperBase::AddLocalMinPoly(Active& e1, Active& e2, const Point64& pt,
                                    bool is_new) {
...
void ClipperBase::DoSplitOp(OutRec* outrec, OutPt* splitOp) {
  ...
  Point64 ip;
  GetSegmentIntersectPt(prevOp->pt, splitOp->pt, splitOp->next->pt,
                        nextNextOp->pt, ip);
...
void ClipperBase::FixSelfIntersects(OutRec* outrec) {
  ...
  for (;;) {
    if (SegmentsIntersect(op2->prev->pt, op2->pt, op2->next->pt,
                          op2->next->next->pt)) {
      if (SegmentsIntersect(op2->prev->pt, op2->pt, op2->next->next->pt,
...
void ClipperBase::IntersectEdges(Active& e1, Active& e2, const Point64& pt) {
  ...
  if (has_open_paths_ && (IsOpen(e1) || IsOpen(e2))) {
    ...
    switch (cliptype_) {
      case ClipType::Union:
        ...
        break;
      default:
        if (edge_c->local_min->polytype == PathType::Subject) return;
    }
  ...
  if (IsHotEdge(e1) && IsHotEdge(e2)) {
    ...
    if (!IsSamePolyType(e1, e2)) {
      ...
      switch (cliptype_) {
        case ClipType::Union:
          if (e1Wc2 <= 0 && e2Wc2 <= 0) AddLocalMinPoly(e1, e2, pt, false);
          break;
        case ClipType::Difference:
...
void ClipperBase::ConvertHorzSegsToJoins() {
  ...
  std::stable_sort(horz_seg_list_.begin(), horz_seg_list_.end(),
                   HorzSegSorter());
...
void ClipperBase::ProcessHorzJoins() {
  for (const HorzJoin& j : horz_join_list_) {
    ...
    } else  // joining, not splitting
    {
      ...
      if (using_polytree_) {
        SetOwner(or2, or1);
        if (or2->splits) MoveSplits(or2, or1);  // #618
      } else
        or2->owner = or1;
    }
  }
...
void ClipperBase::CheckJoinLeft(Active& e, const Point64& pt,
                                bool check_curr_x) {
  Active* prev = e.prev_in_ael;
  if (!prev || !IsHotEdge(e) || !IsHotEdge(*prev) || IsHorizontal(e) ||
      IsHorizontal(*prev) || IsOpen(e) || IsOpen(*prev))
    return;
...
void ClipperBase::CheckJoinRight(Active& e, const Point64& pt,
                                 bool check_curr_x) {
  Active* next = e.next_in_ael;
  if (!next || !IsHotEdge(e) || !IsHotEdge(*next) || IsHorizontal(e) ||
      IsHorizontal(*next) || IsOpen(e) || IsOpen(*next))
    return;
...
bool ClipperBase::CheckSplitOwner(OutRec* outrec, OutRecList* splits) {
  for (auto split : *splits) {
    if (!split->pts && split->splits && CheckSplitOwner(outrec, split->splits))
      return true;  // #942
    split = GetRealOutRec(split);
    if (!split || split == outrec || split->recursive_split == outrec) continue;
    split->recursive_split = outrec;  // prevent infinite loops

    if (split->splits && CheckSplitOwner(outrec, split->splits)) return true;
...
void ClipperBase::RecursiveCheckOwners(OutRec* outrec, PolyPath* polypath) {
  ...
  if (outrec->owner) {
    if (!outrec->owner->polypath) RecursiveCheckOwners(outrec->owner, polypath);
...
bool BuildPathD(OutPt* op, bool reverse, bool isOpen, PathD& path,
                double inv_scale) {
  if (!op || op->next == op || (!isOpen && op->next == op->prev)) return false;
