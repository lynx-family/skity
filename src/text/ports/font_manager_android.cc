 ...
 public:
  explicit FontStyleSetAndroid(const FontFamily& family) {
    ...
    for (size_t index = 0; index < family.fonts.size(); ++index) {
      ...
      std::optional<VariationPosition> last_position = std::nullopt;
    ...
    if (Settings::GetSettings().IsAnyWeightEnabled() && variation_count == 1 &&
        ...
        if (variation_url == full_path) {
          ...
          for (auto& axis : font_file.axis_tags) {
            if (axis.first == "ital" && axis.second == 1.f) {
              ...
              weight_mapping_ptr = &wegiht_mapping_lists[1];
            }
      ...
      std::function<bool(const std::vector<VariationPosition>&)>
          check_posotions =
              [](const std::vector<VariationPosition>& posotions) -> bool {
        if (posotions.size() == 0) {
          return true;
        }
        const VariationPosition& base_pos = posotions[0];
        for (auto& pos : posotions) {
          if (pos != base_pos) {
            return false;
          }
        }
      ...
      if (check_posotions(position_lists[0]) &&
          check_posotions(position_lists[1])) {
        has_same_axes = true;
        ...
        sort_mapping(wegiht_mapping_lists[0]);
        sort_mapping(wegiht_mapping_lists[1]);
        weight_mappings_ = std::move(wegiht_mapping_lists);
      }
    }
