...
void HWDrawStep::GenerateCommand(const HWDrawStepContext& ctx, Command* cmd,
  ...
  if (fragment_->GetProgrammableBlending()) {
    fragment_->GetProgrammableBlending()->SetupBindGroup(cmd, ctx.context);
  }
}
...
GPURenderPipeline* HWDrawStep::GetPipeline(HWDrawContext* context,
  ...
  if (RequireColorWrite()) {
    ...
    pipeline.blend_formula = blend_plan.formula;
  } else {
  ...
  shader_writer_.SetBlendOutputs(blend_plan.formula.primary_output,
                                 blend_plan.formula.secondary_output);
