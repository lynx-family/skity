...
static std::string ResolveBlendModeGoldenFileName(std::string_view file_name) {
  if (file_name ==
          "blend_mode_composite_framebuffer_fetch_sample_count_4.png" ||
  ...
  if (file_name ==
      ...
      file_name ==
          "blend_mode_savelayer_clip_rect_framebuffer_fetch_sample_count_1."
          "png" ||
      file_name ==
          "blend_mode_savelayer_clip_rect_framebuffer_fetch_sample_count_4."
          "png" ||
      file_name ==
  ...
  if (file_name == "blend_mode_clip_replay_texture_copy_sample_count_1.png" ||
      file_name ==
          "blend_mode_savelayer_clip_replay_framebuffer_fetch_sample_count_1."
          "png" ||
      file_name ==
  ...
  if (file_name == "blend_mode_clip_replay_texture_copy_sample_count_4.png" ||
      file_name ==
          "blend_mode_savelayer_clip_replay_framebuffer_fetch_sample_count_4."
          "png" ||
      file_name ==
  ...
  if (IsGLBackend()) {
    if (file_name == "blend_mode_texture_copy_sample_count_4_Darken.png") {
      return "blend_mode_Darken.png";
    }

    if (file_name == "blend_mode_texture_copy_sample_count_4_Multiply.png") {
      return "blend_mode_Multiply.png";
    }
  } else {
    if (file_name == "blend_mode_texture_copy_sample_count_4_Luminosity.png") {
      return "blend_mode_Luminosity.png";
    }

    if (file_name == "blend_mode_texture_copy_sample_count_4_Saturation.png") {
      return "blend_mode_Saturation.png";
    }
  }
...
static std::filesystem::path MakeBlendModeGoldenPath(
...
static skity::testing::GoldenTestEnvConfig DirectSurfaceConfig() {
  auto config = TextureCopyConfig(1);
  config.gl_surface_mode = skity::GLSurfaceMode::kDirect;
  config.gl_has_stencil_attachment = true;
  return config;
}
...
static void RunBlendModeDrawTextureModeRootTextureCopyTest(
  ...
  if (env == nullptr || env->GetBackend() != skity::testing::Backend::kGL) {
    GTEST_SKIP()
        << "DrawTextureSurfaceGL mode override is only validated on GL";
  }
...
static void RunBlendModeDirectModeRootTextureCopyTest(skity::BlendMode mode,
                                                      const char* name) {
  auto* env = skity::testing::GoldenTestEnv::GetInstance();
  if (env == nullptr || env->GetBackend() != skity::testing::Backend::kGL) {
    GTEST_SKIP() << "DirectSurfaceGL mode override is only validated on GL";
  }
  ...
  auto golden_path = MakeBlendModeGoldenPath(
      std::string("blend_mode_texture_copy_sample_count_1_") + name + ".png");
  auto dl = recorder.FinishRecording();
  EXPECT_TRUE(skity::testing::CompareGoldenTexture(
      dl.get(), 100.f, 100.f, golden_path.c_str(), DirectSurfaceConfig()));
}
...
static void RunBlendModeTextureCopyTest(skity::BlendMode mode, const char* name,
                                        uint32_t sample_count) {
  skity::PictureRecorder recorder;
  recorder.BeginRecording(skity::Rect::MakeWH(100.f, 100.f));
  auto canvas = recorder.GetRecordingCanvas();
...
TEST(BlendModeGolden, DirectMode_TextureCopySampleCount1_Overlay) {
  RunBlendModeDirectModeRootTextureCopyTest(skity::BlendMode::kOverlay,
                                            "Overlay");
}
...
static void RunBlendModeSaveLayerTest(skity::BlendMode mode, const char* name) {
  ...
  auto golden_path = MakeBlendModeGoldenPath(
      std::string("blend_mode_savelayer_") + name + ".png");
  auto dl = recorder.FinishRecording();
  EXPECT_TRUE(skity::testing::CompareGoldenTexture(
      dl.get(), 100.f, 100.f,
      skity::testing::PathList{.cpu_tess_path = golden_path.c_str(),
...
static void DrawBlendModeRestoredClipPathTextureCopy(skity::Canvas* canvas) {
  skity::Paint paint;
  paint.SetColor(skity::ColorSetARGB(255, 250, 250, 250));
  canvas->DrawRect(skity::Rect::MakeWH(128.f, 128.f), paint);
  ...
  canvas->Save();
  skity::Path clip_path;
  clip_path.AddCircle(38.f, 38.f, 22.f);
  canvas->ClipPath(clip_path, skity::Canvas::ClipOp::kIntersect);

  paint.SetBlendMode(skity::BlendMode::kSrcOver);
  paint.SetColor(skity::ColorSetARGB(220, 255, 193, 7));
  canvas->DrawRect(skity::Rect::MakeLTRB(8.f, 8.f, 72.f, 72.f), paint);
  canvas->Restore();

  paint.SetBlendMode(skity::BlendMode::kOverlay);
  paint.SetColor(skity::ColorSetARGB(220, 22, 150, 243));
  canvas->DrawRect(skity::Rect::MakeLTRB(44.f, 20.f, 120.f, 108.f), paint);
}
...
static void RunBlendModeRestoredClipPathTextureCopyTest(
    const char* path_name, skity::testing::GoldenTestEnvConfig config) {
  skity::PictureRecorder recorder;
  recorder.BeginRecording(skity::Rect::MakeWH(128.f, 128.f));
  auto canvas = recorder.GetRecordingCanvas();

  DrawBlendModeRestoredClipPathTextureCopy(canvas);
...
TEST(BlendModeGolden, ClipReplayTextureCopySampleCount1) {
  RunBlendModeClipReplayTest(
      "blend_mode_clip_replay_texture_copy_sample_count_1.png",
      TextureCopyConfig(1));
}

TEST(BlendModeGolden, ClipReplayTextureCopySampleCount4) {
  RunBlendModeClipReplayTest(
      "blend_mode_clip_replay_texture_copy_sample_count_4.png",
      TextureCopyConfig(4));
}

TEST(BlendModeGolden, RestoredClipPathTextureCopySampleCount1) {
  RunBlendModeRestoredClipPathTextureCopyTest(
      "blend_mode_restored_clip_path_texture_copy_sample_count_1.png",
      TextureCopyConfig(1));
}
...
TEST(BlendModeGolden, ClipRectTextureCopySampleCount1) {
  RunBlendModeClipRectTest(
      "blend_mode_clip_rect_texture_copy_sample_count_1.png",
      TextureCopyConfig(1));
}

TEST(BlendModeGolden, ClipRectTextureCopySampleCount4) {
  RunBlendModeClipRectTest(
      "blend_mode_clip_rect_texture_copy_sample_count_4.png",
      TextureCopyConfig(4));
}
