...
TEST(CapiTextDelegate, CreateSimpleRejectsEmptyList) {
  ...
  // A zero face count short-circuits before touching the array, so this must
  // reject cleanly even when the only "face" is a null handle (the default
  // typeface is not available on every platform).
  skity_typeface dummy = nullptr;
  EXPECT_EQ(skity_typeface_delegate_create_simple(&dummy, 0), nullptr);
}
...
TEST(CapiTextDelegate, NullDelegateBehavesLikePlainCreate) {
  ...
  if (tf.t == nullptr) {
    GTEST_SKIP() << "default typeface unavailable; skipping";
  }
  PaintGuard paint;
  ...
  BlobGuard with_null{
      skity_text_blob_create_with_delegate("abc", paint.p, nullptr)};
